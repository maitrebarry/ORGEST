...
<body>
    ...
    <div class="doc-title">Facture d'achat d'or</div>
    <div class="numero">N° {{ $achat->numero }} — {{ $achat->date_operation->format('d/m/Y à H:i') }}</div>

    <table class="info-table">
        <tr>
            <td class="label">Client</td><td>{{ $achat->client->nom_complet }}</td>
            <td class="label">Téléphone</td><td>{{ $achat->client->telephone ?? '—' }}</td>
        </tr>
        <tr>
            <td class="label">Prix de base</td><td>{{ number_format($achat->prix_base, 0, ',', ' ') }} {{ $devise }}</td>
            <td class="label">Nombre de barres</td><td>{{ $achat->barres->count() }}</td>
        </tr>
        <tr>
            <td class="label">Opérateur</td><td>{{ $achat->user?->name ?? '—' }}</td>
            <td class="label">Statut</td><td>{{ $achat->statut_libelle }}</td>
        </tr>
    </table>
        ...
        <tbody>
            @foreach ($achat->barres->sortByDesc('poids') as $barre)
                <tr>
                    ...
                    <td>{{ $fmt($barre->prix_unitaire) }}</td>
                    <td>{{ $fmt($barre->montant) }}</td>
                </tr>
            @endforeach
        </tbody>
        <tfoot>
            <tr>
                ...
                <td colspan="2"></td>
                <td colspan="2">{{ $fmt($achat->montant_total) }}</td>
            </tr>
        </tfoot>
    </table>
    ...
    <table class="reglement">
        <tr>
            @if ($afficherBambara)
                <td class="label">Montant total</td><td class="valeur">{{ $fmt($achat->montant_total) }}</td>
                <td class="label">Bambara</td><td class="valeur">{{ $fmt($bambara) }}</td>
            @else
                <td class="label">Montant total</td><td class="valeur" colspan="3">{{ $fmt($achat->montant_total) }}</td>
            @endif
        </tr>
        <tr>
            <td class="label">Montant payé</td><td class="valeur">{{ $fmt($achat->montant_paye) }}</td>
            <td class="label reste">Reste à payer</td><td class="valeur reste">{{ $fmt($achat->reste()) }}</td>
        </tr>
    </table>
    ...
    <table class="signatures">
        <tr>
            ...
            <td><div class="sig-line">{{ $achat->user?->name ?? "L'opérateur" }}</div></td>
        </tr>
    </table>

    <div class="footer">Facture {{ $achat->numero }} — {{ $entreprise['nom'] }}</div>
</body>
</html>
